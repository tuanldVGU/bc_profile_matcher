...
function Movies() {
  ...
  const isSelected = (str: string) => selected.includes(str)

  const uniqueInsert = (str: string) => {
    
    const unique : boolean = selected.every(x => str !== x)
  ...
  const submit = () => {
      (async () => {
        ...
        axios.post(url, {
          ...
          const intersectionSize: number = client?.getIntersectionSize(
